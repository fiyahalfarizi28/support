...
class Rfp_controller extends CI_Controller {
    ...
    public function set_update_it()
    {
        ...
        $exe = $this->db->update(TB_RFP, $arrUpdate);

        if(!$exe)
        {
            $isValid = 0;
            $isPesan = "<div class='alert alert-danger'>RFP Gagal di Simpan</div>";
        }
        else
        {
            $isValid = 1;
            $isPesan = "<div class='alert alert-success'>RFP Berhasil di Simpan</div>";
        }
